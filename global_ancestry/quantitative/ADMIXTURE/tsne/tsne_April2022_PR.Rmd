...
```{r}
Q1 <- read.table("HGDP_1000GP_ORIEN-PR_isec_Apr24_impute_refsonly.20.Q", head = FALSE,  sep = " ", stringsAsFactors = FALSE)
raw_fam1 <- read.table("HGDP_1000GP_ORIEN-PR_isec_Apr24_impute_refsonly.fam", head = FALSE, sep = " ", stringsAsFactors = FALSE)

  #LD version
#Q1 <- read.table("HGDP_1000GP_merge_then_isec_with_ORIEN-PR_prune6_justrefs.20.Q", head = FALSE,  sep = " ", stringsAsFactors = FALSE)
#raw_fam1 <- read.table("HGDP_1000GP_merge_then_isec_with_ORIEN-PR_prune6_justrefs.fam", head = FALSE, sep = " ", stringsAsFactors = FALSE)
...
ORIEN+TCGA --> do run for PR
```{r}
Q2 <- read.table("HGDP_1000GP_ORIEN-PR_isec_Apr24_impute_PRonly.20.Q", head = FALSE,  sep = " ", stringsAsFactors = FALSE)
raw_fam2 <- read.table("HGDP_1000GP_ORIEN-PR_isec_Apr24_impute_PRonly.fam", head = FALSE, sep = " ", stringsAsFactors = FALSE)

  #LD version
#Q2 <- read.table("HGDP_1000GP_merge_then_isec_with_ORIEN-PR_prune6_justPR.20.Q", head = FALSE,  sep = " ", stringsAsFactors = FALSE)
#raw_fam2 <- read.table("HGDP_1000GP_merge_then_isec_with_ORIEN-PR_prune6_justPR.fam", head = FALSE, sep = " ", stringsAsFactors = FALSE)
...
Ethnicity$pop <- "Puerto Rican (ORIEN)"
HGDP_1000GP2 <- HGDP_1000GP[,c(1,2,4:23)]
df <- rbind(Ethnicity,HGDP_1000GP2)
df_sorted <- df[order(df$pop),]
data_tsne <- df_sorted
write.csv(data_tsne,"tsne_samples_ref_info.csv")
...
D=2 #or 3
P=15
tsne_model_1 = Rtsne(as.matrix(data_tsne[,c(3:22)]), check_duplicates=FALSE, pca=TRUE, perplexity=P, theta=0.0, dims=D)
...
d_tsne_1_original$pop_label <- factor(d_tsne_1_original$pop_label, levels = c("African Ancestry SW", 	"African Caribbean", 	"Bantu Herero", 	"Bantu Kenya", 	"Bantu South Africa", 	"Bantu Tswana", 	"Biaka", 	"Esan", 	"Gambian Mandinka", 	"Ju|'hoan North", 	"Ju|'hoan North,San", 	"Luhya", 	"Mandenka", 	"Mbuti", 	"Mbuti,Mbuti", 	"Mende", 	"Mozabite", 	"San", 	"Yoruba", 	"Mozabite,Mozabite", 	"Colombian", 	"Karitiana", 	"Maya", 	"Mayan,Maya", 	"Mexican Ancestry", 	"Peruvian", 	"Piapoco", 	"Pima", 	"Surui", 	"Mongola", 	"Yakut", 	"Balochi", 	"Bengali", 	"Bengali,Bengali", 	"Brahui", 	"Burusho", 	"Gujarati", 	"Hazara", 	"Kalash", 	"Makrani", 	"Pathan", 	"Pathan,Pathan", 	"Punjabi", 	"Punjabi,Punjabi", 	"Sindhi", 	"Tamil", 	"Telugu", 	"Uygur", 	"Cambodian", 	"Cambodian,Cambodian", 	"Dai", 	"Dai Chinese", 	"Daur", 	"Han", 	"Han Chinese", 	"Hezhen", 	"Japanese", 	"Japanese,Japanese", 	"Kinh Vietnamese", 	"Kinh,Kinh Vietnamese", 	"Lahu", "Puerto Rican",	"Miao", 	"Mongolian", 	"Naxi", 	"Northern Han", 	"Oroqen", 	"She", 	"Southern Han Chinese", 	"Tu", 	"Tujia", 	"Xibo", 	"Yi", 	"Adygei", 	"Basque", 	"Bedouin", 	"Bedouin B", 	"Bergamo", 	"Bergamo Italian", 	"Puerto Rican (ORIEN)", "British", 	"British,English", 	"CEPH", 	"Druze", 	"Finnish", 	"Finnish,Finnish", 	"French", 	"Iberian", 	"Iberian,Spanish", 	"Orcadian", 	"Palestinian", 	"Russian", 	"Sardinian", 	"Toscani", 	"Tuscan", 	"Bougainville", 	"Papuan", 	"Papuan Sepik", 	"Papuan,Papuan Highlands", 	"Papuan,Papuan Sepik"), ordered = TRUE)
...
p <- ggplot(d_tsne_1_original, aes_string(x="V1", y="V2", color="pop_label")) +
  ...
  theme(axis.text.x=element_blank(),
       ...
       legend.box = "horizontal", legend.key.size = unit(0.5, "lines"), legend.key.width= unit(1, 'cm')) + 
...
p <- p + ggtitle("TSNE of Admixture Results for K=20; D=2, Seed=9, Perplexity=15, PCA=TRUE") + theme(plot.title = element_text(size=10, face="bold.italic")) + theme(plot.title = element_text(hjust = 0.5))
p
ggsave("K20_seed9_D2_perplexity15_admixture_TSNE_25Apr2022_PR_PCAtrue.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
#ggsave("K20_seed9_D2_perplexity55_admixture_TSNE_22Apr2022_PR_PCAtrue_LD6.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
...
p2 <- p + geom_point(data=PUR, 
             ...
             size=0.05) 
p2
#ggsave("K20_seed9_D2_perplexity55_admixture_TSNE_22Apr2022_PCAtrue_LD6_PUR.jpeg", width=10, height=5, units="in", res=1000, device = jpeg)
ggsave("K20_seed9_D2_perplexity15_admixture_TSNE_25Apr2022_PCAtrue_PUR.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
...
p3 <- p + geom_point(data=PUR2, 
             ...
             size=0.05)
p3
ggsave("K20_seed9_D2_perplexity15_admixture_TSNE_25Apr2022_PUR-ORIEN.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
...
p3 <- p + geom_point(data=MXL, 
             ...
             size=0.05)
p3
ggsave("K20_seed9_D2_perplexity15_admixture_TSNE_25Apr2022_MXL.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
...
p4 <- p + geom_point(data=PEL, 
             ...
             size=0.05)
p4
ggsave("K20_seed9_D2_perplexity15_admixture_TSNE_25Apr2022_PEL.jpeg", width=10, height=10, units="in", res=1000, device = jpeg)
```
